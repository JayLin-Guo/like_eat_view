...
用户实时特征向量：
- 静态画像：人群属性 [\${occupation}] | 地域或口味倾向 [\${region}]
- 动态上下文：日历进度 [\${dayType}] | 场景 [\${mealTime}]
- 即时决策反馈：[\${history}]
召回策略分配规则（务必混合以下策略）：
1. 🎯 【精准内容召回】：直接命中其“当下灵魂回答”与“人群限制条件”（例如：如果他是健身自律党，绝对不给炸鸡；如果他是职场白领且在工作日，优选上菜快、能量密度合理的）。
2. 👥 【协同过滤召回】：该地域/场景/人群下，大众最常点的高频爆款（如：周末+深夜食堂=大放纵烧烤）。
3. 🎲 【探索与利用机制 (E&E)】：为了防止信息房茧房，注入 2 个【长尾/跨界】但极具诱惑力的小众类目。
要求：
1. 类别名称需高度标准化且带有你**当前角色设定**的强烈情感色彩煽动性（如白领风：“满分充能营养轻食”；狂欢风：“周末深夜罪恶烧烤场”）。
...
2. 【解释性推荐】：在描述中，加入**符合你当前角色语气**的强烈感官描述。同时根据用户的人群属性与周末工作日特征，提供针对性的推荐背书。
3. 【属性打标 (Tags)】：生成 2-3 个核心标签（包含：口味特征、转化卖点，如 "#工作日回血", "#周末极致放纵", "#优质蛋白"）。
